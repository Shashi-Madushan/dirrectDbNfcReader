...
void setup() {
  ...
  pinMode(D3, OUTPUT);
  digitalWrite(D3, LOW); // Initial state for buzzer 
  ...
  if(WiFi.status() == WL_CONNECTED){
    ...
    if ((currentHour >= startHour1 && currentHour < endHour1) ||
      ...
      if (!mfrc522.PICC_IsNewCardPresent()) {
        setLEDColor(2, strip.Color(255, 0, 0)); // No card present
        setLEDColor(3, strip.Color(255, 0, 0)); // No card present
        delay(50);
        return;
      }
      ...
      if (cardData != "") {
        ...
        if (conn.connected()) {
          ...
          setLEDColor(2, strip.Color(0, 255, 0)); // Card present, data saved (green)
          setLEDColor(3, strip.Color(0, 255, 0)); // Card present, data saved (green)
        } else {
          Serial.println("Not connected to MySQL server.");
          setLEDColor(2, strip.Color(255, 225, 255)); // Card present, data not saved 
          setLEDColor(3, strip.Color(255, 225, 225)); // Card present, data not saved 
        }
      ...
      setLEDColor(2, strip.Color(255, 0, 0)); // Out of time range (red)
      setLEDColor(3, strip.Color(255, 0, 0)); // Out of time range (red)
    }
  }else{
    connectToWiFi();
  }
...
void connectToWiFi() {
  ...
  while (WiFi.status() != WL_CONNECTED && millis() - startAttemptTime < 10000) {
    delay(500);
    Serial.print(".");
    blinkLED(0, strip.Color(255, 255, 255), 500);
  }
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    setLEDColor(0, strip.Color(0, 0, 255)); // Solid blue
  } else {
...
void ensureMySQLConnection() {
  if (!conn.connected()) {
    ...
    while (!conn.connect(ip, db_port, db_user, db_password, db_name)) {
      blinkLED(1, strip.Color(0, 225, 0), 500); // green blinking
    }
    Serial.println("MySQL connection successful.");
    setLEDColor(1, strip.Color(0, 0, 255)); // Solid blue
  } else if(conn.connected()) {
    Serial.println("MySQL connection failed.");
    setLEDColor(1, strip.Color(0, 0, 255)); // Solid blue
  }else{
    Serial.println("MySQL connection failed.");
    setLEDColor(1, strip.Color(255, 0, 0)); // Solid red
  }
}
